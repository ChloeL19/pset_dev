...
# alternative ipython start: %run ./main_staff_comfy.py

# import keras library
import keras
import os
# import our helper files
from model_architecture_staff_comfy import *
...
# cut down on the logging from tensorflow
...
# figuring out an appropriate name for this run based on tensorboard_utils function
version = name_run(dataset)
model, tensorboard = create_model(X_train.shape[1], X_train.shape[2], X_train.shape[3], Y_train.shape[1], run_version=version)
...
print("Setting up tensorboard for visualization purposes . . . ")
# start up tensorboard for visualization
# suppress the output of this function
...
# copy the correct port to the clipboard 
...
batch_size = 128
epochs = 10
# record the training history of the model
# and train the model with tensorboard 
train_history = model.fit(X_train, Y_train, batch_size=batch_size, epochs=epochs, validation_data=(X_test, Y_test), callbacks=[tensorboard])
# ask if the user wants to train the model again
val_resp = False
while val_resp == False:
    train_again = input("Would you like to train the model again? ").lower()
...
while (valid == False):
    ...
    if choice in yes:
        ...
        # clear that particular run file in the logs directory (should we?)
        #TODO
        valid = True
    else:
       print("Please respond with 'yes' or 'no'")
